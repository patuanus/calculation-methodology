        ...
        function ButtonHoiQuyValueChanged(app, event)
            ...
            % hoi quy tuyen tinh
             if strcmp(pp,'TuyenTinh')
                 ...
                 sr = 0;
                    for i = 1:n
                        sumx = sumx + X(i);     %tong gia tri cua x
                        sumy = sumy + Y(i);     %tong gia tri cua y
                        sumxy = sumxy + X(i) * Y(i); %tong gia tri x*y
                        sumx2 = sumx2 + X(i) * X(i); %tong gia tri x*x
                    end
                 xm = sumx/n;       %gia tri trung binh cua x
                 ym = sumy/n;       %gia tri trung binh cua y
                 a1 = (n*sumxy -sumx*sumy)/(n*sumx2-sumx*sumx); %gia tri a
                 a0 = ym - (a1 * xm);   %gia tri b
